...
static __always_inline int ipv4_redirect_handle(struct xdp_md *ctx, struct ethhdr *ethh) {
	...
	lhinfo = bpf_map_lookup_elem(&xdp_redirect_map, &idx);
...
SEC("xdp")
int xdp_lhredirect(struct xdp_md *ctx) {
	void *data_end = (void *)(long)ctx->data_end;
