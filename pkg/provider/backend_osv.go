...
func (b *OSvBackend) CreateInstance(instance *Instance) error {
	...
	netDev, internalIP, internalMac, err := (*network.PodNetworkManager).CreateVMTun(instance.Metadata.Namespace, instance.Metadata.Name, instance.Name)
	if err != nil {
		log.L.Errorf("Error setting up network for %s/%s %v", instance.Metadata.Name, instance.Name, err)
	}

	cmd := []string{}
	if netDev != "" {
		gateway := (*network.PodNetworkManager).GetGatewayAddress(instance.Metadata.Namespace, instance.Metadata.Name)
		cmd = []string{fmt.Sprintf("--ip=eth0,%s,255.255.255.0 --defaultgw=%s", internalIP.String(), gateway)}
	}
	// Get hypervisor options
	// Container.Image
	image := b.imageDiskPath(instance.Image, imageConf.Hypervisor)
	// Container.Command
	cmd = append(cmd, instance.Command...)
	cmd = append(cmd, instance.Args...)
	if cmd == nil || len(cmd) == 0 {
		cmd = []string{"runscript", "/run/default;"}
	}
	// Container.WorkingDir (TODO)
	// Container.Ports
	// Get a TAP device from the container networking, if nil then we continue with a bridge as usual
	// (legacy and possibly future modes just use default qemu bridge)
	networking := "tap"
	//networking := "bridge"
	if netDev == "" {
		networking = "bridge"
	...
	//If we have a TAP device, don't process natRules
	...
	// But we do need the mac address for the internal device, which the library doesn't support atm.. hmm
